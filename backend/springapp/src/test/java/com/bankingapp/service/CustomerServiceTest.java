...
class CustomerServiceTest {
	...
	public void find_customerId_OK() throws Exception{
		...
		mockMvc.perform(get("/customer/customer/101"))
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        ...
        .andExpect(jsonPath("$.contact", is(9968101)));
    ...
    public void find_customerIdNotFound_404() throws Exception {
        mockMvc.perform(get("/customer/customer/100")).andExpect(status().isNotFound());
    }
    ...
    public void delete_customer_OK() throws Exception {
        ...
        doNothing().when(customerRepository).deleteById(101);

        mockMvc.perform(delete("/customer/customers/101"))
                .andExpect(status().isOk());

        verify(customerRepository, times(1)).deleteById(101);
    }
}
